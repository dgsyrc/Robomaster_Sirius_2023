...
// 加上数据包的包头 和CRC32 78字节（4+70+4）
typedef struct
{
   COMHead head;    //数据包头
   ServoComdV3 cmd; //数据体（看A1手册）
   uint32_t CRCdata; //CRC32校验
} MotorData_t;  //返回数据

typedef struct
{                               // 以 4个字节一组排列 ，不然编译器会凑整
                                // 定义 数据
    uint8_t mode;               // 关节模式选择 0：停转 5：开环慢转 10：闭环伺服控制
...
{
    // 定义 电机控制命令数据包
    COMHead head;   // 包头         4Byte
    MasterComdV3 cmd;   //数据体
    uint32_t   CRC32;   // CRC          4Byte
