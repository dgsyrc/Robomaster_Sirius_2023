...
extern VisionDataGet_Typedef  visionDataGet;
/**
...
//小陀螺模式
static void gimbal_xtl_control(fp32 *yaw, fp32 *pitch, gimbal_control_t *gimbal_control_set);
	...
	void gimbal_behaviour_mode_set(gimbal_control_t *gimbal_mode_set)
{
    ...
    else if (gimbal_behaviour == GIMBAL_ABSOLUTE_ANGLE)
    {
        ...
        gimbal_mode_set->gimbal_pitch_motor.gimbal_motor_mode = GIMBAL_MOTOR_GYRO;   //不知道啥原理改为相对角后yaw动pitch跟着动
    }
	...
	else if (gimbal_behaviour == GIMBAL_XTL)  //小陀螺
	{
		gimbal_mode_set->gimbal_yaw_motor.gimbal_motor_mode = GIMBAL_MOTOR_GYRO;       //yaw轴绝对角度控制
		gimbal_mode_set->gimbal_pitch_motor.gimbal_motor_mode = GIMBAL_MOTOR_GYRO;  //pitch轴相对角度控制
	}		
}
...
void gimbal_behaviour_control_set(fp32 *add_yaw, fp32 *add_pitch, gimbal_control_t *gimbal_control_set)
{
    ...
    else if (gimbal_behaviour == GIMBAL_MOTIONLESS)
    {
		...
		else if (gimbal_behaviour == GIMBAL_XTL)
		{
			  gimbal_xtl_control(add_yaw, add_pitch, gimbal_control_set);
		} 
...
//					xtl_open_flag = 1;
//				}
//				else xtl_open_flag = 0;
//				
//				if(xtl_open_flag && (!last_xtl_open_flag))
//				{
//					xtl_key_count++;
//				}
//				
...
//					xtl_flag = 0;
//				}
//				last_xtl_open_flag = xtl_open_flag;
//				
//				if(get_remain_HP() == 0)
//				{
//					xtl_key_count = 0;
//				}
	if(gimbal_mode_set->gimbal_rc_ctrl->key.v & KEY_PRESSED_OFFSET_C || switch_is_mid(gimbal_mode_set->gimbal_rc_ctrl->rc.s[GIMBAL_MODE_CHANNEL]) )  //按C键进入小陀螺
	{
		xtl_flag = 1;
	}
//	else
...
//			gimbal_behaviour = GIMBAL_ABSOLUTE_ANGLE;
    ...
    //判断进入init状态机
    {
        static gimbal_behaviour_e last_gimbal_behaviour = GIMBAL_ZERO_FORCE;   //之前状态
		//如果之前是无力状态，进入其他状态需要首先进行初始化
...
//		  count_Sendtosight++;
////		gimbal_control_set ->gimbal_yaw_motor.absolute_angle_set = gimbal_control_set ->gimbal_yaw_motor.absolute_angle_set-0.08f*ChariotRecognition_yaw;// + (ChariotRecognition_yaw - last_ChariotRecognition_yaw)/20 * count_Sendtosight;
////		gimbal_control_set ->gimbal_pitch_motor.relative_angle_set = gimbal_control_set ->gimbal_pitch_motor.relative_angle_set-0.1f*ChariotRecognition_pitch;// + (ChariotRecognition_pitch - last_ChariotRecognition_pitch)/20 * count_Sendtosight;
		//	gimbal_control_set ->gimbal_pitch_motor.absolute_angle_set = ChariotRecognition_pitch;// + (ChariotRecognition_pitch - last_ChariotRecognition_pitch)/20 * count_Sendtosight;
	gimbal_control_set ->gimbal_yaw_motor.absolute_angle_set += visionDataGet.yaw_angle.yaw_predict/20000.0/57.32f;          //5000
//	gimbal_control_set ->gimbal_pitch_motor.relative_angle_set += visionDataGet.pitch_angle.pitch_predict/10.0/57.32f;      
	gimbal_control_set ->gimbal_pitch_motor.absolute_angle_set += visionDataGet.pitch_angle.pitch_predict/50000.0/57.32f;   //35750
//			*yaw = -ChariotRecognition_yaw;//视觉yaw轴角度增量     
//   	  *pitch = -ChariotRecognition_pi+tch;//视觉pitch轴增量角度
//		visionDataGet.yaw_angle.yaw_predict=0;
//		visionDataGet.pitch_angle.pitch_predict=0;
		ChariotRecognition_yaw = 0.0f;
		ChariotRecognition_pitch = 0.0f ;
	}
...
//    *pitch = pitch_channel * PITCH_RC_SEN + gimbal_control_set->gimbal_rc_ctrl->mouse.y * PITCH_MOUSE_SEN;
    ...
    if(!autoaim_mode_flag)   //无自瞄状态下 一键掉头函数  V键开启
    {
  ...
  //      static uint8_t gimbal_turn_flag = 0;
        ...
        if (gimbal_turn_flag)
        {
            ...
            else if(rad_format(gimbal_end_angle - gimbal_control_set->gimbal_yaw_motor.absolute_angle) < 0.0f)
            {
                *yaw -= TURN_SPEED;
            }
        }
        //到达pi （180°）后停止
        if (gimbal_turn_flag && fabs(rad_format(gimbal_end_angle - gimbal_control_set->gimbal_yaw_motor.absolute_angle)) < 0.08f)
        {
            gimbal_turn_flag = 0;
        }
    }
