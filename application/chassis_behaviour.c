...
//小陀螺
static void chassis_xtl_control(fp32 *vx_set, fp32 *vy_set, fp32 *wz_set, chassis_move_t *chassis_move_rc_to_vector);
...
int CTRL_G_FLAG = 0;
int LAST_CTRL_G_FLAG = 0;
int CHASSIS_CTRL_FLAG = 1;     //0为遥控器控制 1为键盘控制 如果没使用键鼠下面直接改为0
int LAST_CHASSIS_CTRL_FLAG = 1;
void chassis_behaviour_mode_set(chassis_move_t *chassis_move_mode)
{
    ...
    CTRL_G_FLAG = chassis_move_mode->chassis_RC->key.v & KEY_PRESSED_OFFSET_G;
    if(CTRL_G_FLAG && !LAST_CTRL_G_FLAG)
		{
			CHASSIS_CTRL_FLAG = !LAST_CHASSIS_CTRL_FLAG;
			LAST_CHASSIS_CTRL_FLAG = CHASSIS_CTRL_FLAG;
		}
		LAST_CTRL_G_FLAG = CTRL_G_FLAG;
    //remote control  set chassis behaviour mode
    //遥控器设置模式
		
		if(CHASSIS_CTRL_FLAG == 1)
		{
			if(xtl_flag == 1)
			{
				chassis_behaviour_mode = CHASSIS_XTL;
			}
			else
			{
		...
		else if(CHASSIS_CTRL_FLAG == 0)
		{
							if (switch_is_mid(chassis_move_mode->chassis_RC->rc.s[CHASSIS_MODE_CHANNEL]))
							{
									...
									if(chassis_move_mode->chassis_RC->rc.ch[4] == 660||chassis_move_mode->chassis_RC->rc.ch[4] == -660)
									{
										chassis_behaviour_mode = CHASSIS_XTL;
									xtl_flag = 1;
									}
							...
							if(xtl_flag == 1)
							{
								chassis_behaviour_mode = CHASSIS_XTL;  //这个用在键控上，键控时右开关打到最上
							}
									else
							{
									//chassis_behaviour_mode =  CHASSIS_ENGINEER_FOLLOW_CHASSIS_YAW;
								chassis_behaviour_mode = CHASSIS_INFANTRY_FOLLOW_GIMBAL_YAW;//CHASSIS_NO_FOLLOW_YAW;
									...
									// chassis_behaviour_mode = CHASSIS_XTL;
							//chassis_behaviour_mode =  CHASSIS_ENGINEER_FOLLOW_CHASSIS_YAW;//无键盘时 遥控器开启小陀螺
							}
							}
		}
	...
	else if (chassis_behaviour_mode == CHASSIS_XTL)   //小陀螺
	{
		  chassis_move_mode->chassis_mode = CHASSIS_VECTOR_NO_FOLLOW_YAW;
		//  mode_index = 6;
	}
...
void chassis_behaviour_control_set(fp32 *vx_set, fp32 *vy_set, fp32 *angle_set, chassis_move_t *chassis_move_rc_to_vector)
{
    ...
    else if (chassis_behaviour_mode == CHASSIS_OPEN)
    {
		...
		else if (chassis_behaviour_mode == CHASSIS_XTL)
		{
			  chassis_xtl_control(vx_set, vy_set, angle_set, chassis_move_rc_to_vector);
		}
...
int xtl_time;
float changing_speed_function;
RC_ctrl_t *xtl_RC;
extern uint8_t open_power_flag;
static void chassis_xtl_control(fp32 *vx_set, fp32 *vy_set, fp32 *wz_set, chassis_move_t *chassis_move_rc_to_vector)  //注释部分为正弦运动，一般可不取。
{
//	  xtl_time++;      //2ms加1  1秒time加500 
//	
//	  if(xtl_time >= 360)
//		{
//			xtl_time = 0;     //0~360周期变
...
//        return;
//    }
//    if(AHRS_sinf((fp32)(xtl_time * 3.14159 / 180)) <= -0.1)   //正弦函数下限-0.1，可改
//		{
//			changing_speed_function = -0.1;
//		}
//		else if(AHRS_sinf((fp32)(xtl_time * 3.14159 / 180)) > -0.3 && AHRS_sinf((fp32)(xtl_time * 3.14159 / 180)) < 0.8)  //正弦
//		{
//			changing_speed_function = AHRS_sinf((fp32)(xtl_time * 3.14159 / 180));
//		}
//		else if(AHRS_sinf((fp32)(xtl_time * 3.14159 / 180)) >= 0.8)  //正弦函数上限0.8，可改
//		{
//			changing_speed_function = 0.8;
//		}
	  chassis_rc_to_control_vector(vx_set, vy_set, chassis_move_rc_to_vector);
		//控制底盘前后左右
		if(open_power_flag == 1)
		{
		  if(xtl_RC->rc.ch[4] == 660)
	   	{
		    *wz_set = -((float)(6.0 + 0.7 * get_robot_level()));
		  }
		  else if(xtl_RC->rc.ch[4] == -660)
		  {
			  *wz_set = ((float)(6.0 + 0.7 * get_robot_level()));
		  }
		  else
		...
		else if(open_power_flag == 0)
		{
			if(xtl_RC->rc.ch[4] == 660)
	   	{
		    *wz_set = -((float)(5.0 + 0.7 *  get_robot_level()));
		  }
		  else if(xtl_RC->rc.ch[4] == -660)
		  {
			  *wz_set = ((float)(5.0 + 0.7 * get_robot_level()));
