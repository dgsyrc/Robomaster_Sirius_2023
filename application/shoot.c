...
//			 }
//			 if((shoot_control.press_l)||(switch_is_up(shoot_control.shoot_rc->rc.s[SHOOT_RC_MODE_CHANNEL])) || (visionDataGet.shoot_msg == 1))
//			 {
        //设置拨弹轮的拨动速度,并开启堵转反转处理
...
//			 }
    ...
    {
								shoot_control.fric1_speed_set = -2.3f;  //2.65
					      shoot_control.fric2_speed_set = 2.3f;  //2.65 
//			          shoot_control.speed_set = 5.0f;
//			if(get_robot_shoot_speed_limit() == 15)
//			{
//				  shoot_control.fric1_speed_set = -1.65f;  // -1.60f
//				  shoot_control.fric2_speed_set = 1.65f;
//			}
//			else if(get_robot_shoot_speed_limit() == 18)
//			{
//					shoot_control.fric1_speed_set = -1.80f;
//					shoot_control.fric2_speed_set = 1.80f;
//				
...
//			{
//					shoot_control.fric1_speed_set = -3.1f;  //2.65
//					shoot_control.fric2_speed_set = 3.1f;  //2.65 
...
//			else 
//			{
//					shoot_control.fric1_speed_set = -1.65f;
//					shoot_control.fric2_speed_set = 1.65f;
...
//					shoot_control.fric2_speed_set = 1.65 + KEY_TO_FRIC_SPEED_SEN * shoot_speed_add_little;
	//		}
		...
		CAN_cmd_fric(shoot_control.fric1_given_current, shoot_control.fric2_given_current,shoot_control.given_current, 0);   //摩擦轮电机赋值。 
...
//--------------------------------------------1-------------------------------
    ...
    else if ((switch_is_mid(shoot_control.shoot_rc->rc.s[SHOOT_RC_MODE_CHANNEL])))   //开关在中档，鼠标左键按下，自瞄设计信息
    {
    ...
    else if(shoot_control.shoot_mode == SHOOT_READY)
    {
        ...
        if ( (switch_is_up(shoot_control.shoot_rc->rc.s[SHOOT_RC_MODE_CHANNEL]) ) )  //|| (shoot_control.press_l && shoot_control.last_press_l == 0) 
        {
		     // if(get_game_progress() == 4)
	//	        {    
						 get_shoot_heat0_limit_and_heat0(&shoot_control.heat_limit, &shoot_control.heat);
//		
//	修改				      	if(shoot_control.heat + 30 < shoot_control.heat_limit)
//						     	{
						     		shoot_control.shoot_mode = SHOOT_CONTINUE_BULLET;
//						     	}
//			 	         	else 
...
//		}
    ...
    //射击开关下档时间计时
    if (shoot_control.shoot_mode != SHOOT_STOP && switch_is_up(shoot_control.shoot_rc->rc.s[SHOOT_RC_MODE_CHANNEL]))
    {
...
static void shoot_bullet_control(void)  
{
    ...
    {   
        shoot_control.set_angle = shoot_control.angle + PI_TEN;
			  shoot_move_flag_count++;
			  if(shoot_move_flag_count>3)
				{
				  shoot_control.move_flag = 1;
					shoot_move_flag_count=0;
				} 
    }
//    if(shoot_control.key == SWITCH_TRIGGER_OFF)
//    {
...
//    	
    //到达角度判断
    if (rad_format(shoot_control.set_angle - shoot_control.angle) > 0.03f)
    {
        //没到达一直设置旋转速度
    ...
    {
//			if(!last_shoot_bullet_flag)
//			{
			  shoot_control.move_flag = 0;
				last_shoot_bullet_flag = 1;
//			}
