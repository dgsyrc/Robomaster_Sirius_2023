...
/**
  ...
  * @retval         none
  */
ext_game_robot_state_t RM_robot_state;
...
//			UI_ReFreshAll();
			...
			SendVisionData(&visionDataSend);                       //向视觉发送数据
...
static void gimbal_init(gimbal_control_t *init)
{
	...
	//设置pitch轴软件限位
	init->gimbal_pitch_motor.max_software_relative_angle = (8000-init->gimbal_pitch_motor.offset_ecd) * (PI/HALF_ECD_RANGE);      //5060
	init->gimbal_pitch_motor.max_software_relative_angle_xtl = (8000-init->gimbal_pitch_motor.offset_ecd) * (PI/HALF_ECD_RANGE);; //4920
...
static void gimbal_feedback_update(gimbal_control_t *feedback_update)
{
    ...
    feedback_update->gimbal_pitch_motor.motor_gyro = *(feedback_update->gimbal_INT_gyro_point + INS_GYRO_Z_ADDRESS_OFFSET);
...
/**
  ...
  */
/**
  * @brief          设置云台控制设定值（也就是目标值），控制值是通过gimbal_behaviour_control_set函数设置的
  * @param[out]     gimbal_set_control:"gimbal_control"变量指针.
  * @retval         none
  */
static void gimbal_set_control(gimbal_control_t *set_control)
...
//		set_control ->gimbal_yaw_motor.absolute_angle_set = set_control ->gimbal_yaw_motor.absolute_angle_set-visionDataGet.yaw_angle.yaw_predict/5000/57.32f;
//		set_control ->gimbal_pitch_motor.relative_angle_set = set_control ->gimbal_pitch_motor.relative_angle_set+visionDataGet.pitch_angle.pitch_predict/100/57.32f;
    //yaw电机模式控制
...
static void gimbal_yaw_absolute_angle_limit(gimbal_motor_t *gimbal_motor, fp32 add)
{
 //   static fp32 bias_angle;
    static fp32 angle_set;
	  static fp32 max_add;     // 最大角最小角之差除以2
    if (gimbal_motor == NULL)
    {
        return;
    }
	 	max_add = 0.5f * (gimbal_motor->max_relative_angle - gimbal_motor->min_relative_angle);
//		if(servor_real_open_ui == 1)
//		{
//			gimbal_motor->absolute_angle_set = rad_format(gimbal_motor->max_relative_angle - max_add);
//		}
	 if(servor_real_open_ui == 0||servor_real_open_ui==1)
		{
    //now angle error
    //当前控制误差角度
...
//			}
			angle_set = gimbal_motor->absolute_angle_set;
//			if(angle_set < gimbal_motor->min_relative_angle)
//			{
//				angle_set = gimbal_motor->min_relative_angle;
//			}
//			if(angle_set >gimbal_motor->max_relative_angle)
//			{
//				angle_set = gimbal_motor->max_relative_angle;
//			}	
			gimbal_motor->absolute_angle_set = rad_format(angle_set + add);  //yaw轴直接赋值给ref，不用角度增量		
		}
	}
    //gimbal_motor->absolute_angle_set = rad_format(angle_set + add);
...
static void gimbal_absolute_angle_limit(gimbal_motor_t *gimbal_motor, fp32 add)
{
    ...
    static fp32 angle_set;
    static fp32 max_add;     // 最大角最小角之差除以2
	static fp32 max_relative_angle_tmp;  
	//暂未实现的功能：小陀螺模式和跟随模式下限幅不同
//	if(xtl_flag == 1) //如果是小陀螺状态下，使用范围更小的限幅
//	{
//		max_relative_angle_tmp = gimbal_motor->max_software_relative_angle_xtl;
//	}
//	else 
//	{
		max_relative_angle_tmp = gimbal_motor->max_software_relative_angle;
//	}
	...
	max_add = 0.5f * (gimbal_motor->max_relative_angle - gimbal_motor->min_relative_angle);
//	if(servor_real_open_ui == 1)  //弹舱盖开启
//	{
//		gimbal_motor->absolute_angle_set = rad_format(gimbal_motor->max_relative_angle - max_add);
//	}       
    if(servor_real_open_ui == 0||servor_real_open_ui==1)  //弹舱盖关闭
	{
		//now angle error
		//当前控制误差角度
		bias_angle = rad_format(gimbal_motor->absolute_angle_set - gimbal_motor->absolute_angle);
		...
		if (gimbal_motor->relative_angle + bias_angle + add > max_relative_angle_tmp)
		{
			//如果是往最大机械角度控制方向
			if (add > 0.0f)
			{
				//calculate max add_angle
				//计算出一个最大的添加角度，
				add = max_relative_angle_tmp - gimbal_motor->relative_angle - bias_angle;

			}
		}
		//云台相对角度+ 误差角度 + 新增角度 如果大于 反方向最大软件限位角度
		else if (gimbal_motor->relative_angle + bias_angle + add < gimbal_motor->min_relative_angle)
		{
			if (add < 0.0f)
			{
...
static void gimbal_relative_angle_limit(gimbal_motor_t *gimbal_motor, fp32 add)
{
	...
	if(servor_real_open_ui == 1)
	{
		gimbal_motor->relative_angle_set = rad_format(gimbal_motor->max_relative_angle - max_add);
	}
	else if(servor_real_open_ui == 0)
	{
// 		gimbal_motor->relative_angle_set += add;
		...
		else if(autoaim_mode_flag == 1)  //自瞄模式
		{
			gimbal_motor->relative_angle_set += add/1000;  //pitch轴直接赋值给ref，不用角度增量
		}
...
static void gimbal_motor_absolute_angle_control(gimbal_motor_t *gimbal_motor)
{
	...
	if(autoaim_mode_flag == 0)  //正常操作手模式
	{
		if(gimbal_motor == &gimbal_control.gimbal_yaw_motor)
	    {
			gimbal_motor->gimbal_motor_absolute_angle_pid.kp = 26.0f;		         //26
			gimbal_motor->gimbal_motor_absolute_angle_pid.ki = 0.0f;              //0
			gimbal_motor->gimbal_motor_absolute_angle_pid.kd = 0.0f;            

			gimbal_motor->gimbal_motor_gyro_pid.Kp = 5000.0f;       //5000       
			gimbal_motor->gimbal_motor_gyro_pid.Ki = 20.0f;              
			gimbal_motor->gimbal_motor_gyro_pid.Kd = 0.0f;               
		}
		if(gimbal_motor == &gimbal_control.gimbal_pitch_motor)
		{
//			fp32 kp_tmp,kp_max,kp_min;
//			kp_max = 26.0f;
//			kp_min = 19.0f;
//			kp_tmp = gimbal_motor->relative_angle - gimbal_motor->min_relative_angle;
//			kp_tmp *= (kp_max - kp_max) / (gimbal_motor->max_software_relative_angle - gimbal_motor->min_relative_angle);
//			kp_tmp += kp_min;
//			gimbal_motor->gimbal_motor_absolute_angle_pid.kp = kp_tmp;		         //28//26
//			gimbal_motor->gimbal_motor_absolute_angle_pid.ki = 0.0f;              //0
//			gimbal_motor->gimbal_motor_absolute_angle_pid.kd = 0.3f;            //0.3

//			gimbal_motor->gimbal_motor_gyro_pid.Kp = 2300.0f;              //2175 
//			gimbal_motor->gimbal_motor_gyro_pid.Ki = 1.0f;                 //5
//			gimbal_motor->gimbal_motor_gyro_pid.Kd = 0.0f;               //100
			//此处采用分段式PID，以pitch中值为界(加一个偏移量微调)，分别使用不同的参数
			//pitch中值下参数：26.0  0.0  0.3	
			if(gimbal_motor->relative_angle > -0.02) {
				gimbal_motor->gimbal_motor_absolute_angle_pid.kp = 26.0f;		         //28//26低头
				gimbal_motor->gimbal_motor_absolute_angle_pid.ki = 0.0f;              //0
				gimbal_motor->gimbal_motor_absolute_angle_pid.kd = 0.3f;            //0.3
			...
			//pitch中值上参数：                                                        //抬头
				...
				gimbal_motor->gimbal_motor_absolute_angle_pid.kp = 19.0f;		         //28//26
				gimbal_motor->gimbal_motor_absolute_angle_pid.ki = 0.0f;              //0
				gimbal_motor->gimbal_motor_absolute_angle_pid.kd = 0.3f;            //0.3
	...
	else if(autoaim_mode_flag == 1)   //自瞄的pid
	{
		if(gimbal_motor == &gimbal_control.gimbal_yaw_motor)
	    {
			gimbal_motor->gimbal_motor_absolute_angle_pid.kp = 60.0f;		         //26
			gimbal_motor->gimbal_motor_absolute_angle_pid.ki = 0.0f;              //0
			gimbal_motor->gimbal_motor_absolute_angle_pid.kd = 0.0f;             //0

			gimbal_motor->gimbal_motor_gyro_pid.Kp = 8000.0f;              //5000  
			gimbal_motor->gimbal_motor_gyro_pid.Ki = 0.0f;                //20
			gimbal_motor->gimbal_motor_gyro_pid.Kd = 0.0f;               
//			gimbal_motor->motor_gyro_set = gimbal_PID_calc(&gimbal_motor->gimbal_motor_absolute_angle_pid, gimbal_motor->absolute_angle, gimbal_motor->absolute_angle_set, gimbal_motor->motor_gyro);
//      	gimbal_motor->current_set = PID_calc(&gimbal_motor->gimbal_motor_gyro_pid, gimbal_motor->motor_gyro, gimbal_motor->motor_gyro_set);
		}
		if(gimbal_motor == &gimbal_control.gimbal_pitch_motor)
		{
			gimbal_motor->gimbal_motor_absolute_angle_pid.kp = 35.0f;		         //26
			gimbal_motor->gimbal_motor_absolute_angle_pid.ki = 0.0f;              //0
			gimbal_motor->gimbal_motor_absolute_angle_pid.kd = 0.3f;            //0.3
		  
			gimbal_motor->gimbal_motor_gyro_pid.Kp = 1500.0f;              //2900  //3300
			gimbal_motor->gimbal_motor_gyro_pid.Ki = 0.0f;                 //5
			gimbal_motor->gimbal_motor_gyro_pid.Kd = 0.0f;               //100
...
static void gimbal_motor_relative_angle_control(gimbal_motor_t *gimbal_motor)
{
	...
	if(autoaim_mode_flag == 0)  //正常操作手模式
	 {
		 if(gimbal_motor == &gimbal_control.gimbal_yaw_motor)
	    {
		    gimbal_motor->gimbal_motor_relative_angle_pid.kp = 100.0f;   //15 38
				gimbal_motor->gimbal_motor_relative_angle_pid.ki = 0.0f;    //0
				gimbal_motor->gimbal_motor_relative_angle_pid.kd = 0.0f;    //0
				
				gimbal_motor->gimbal_motor_gyro_pid.Kp = 5000.0f; //2900  4500  //3200
				gimbal_motor->gimbal_motor_gyro_pid.Ki = 20.0f;      //60       //5
				gimbal_motor->gimbal_motor_gyro_pid.Kd = 0.0f;       //0     //100			
			}
		 if(gimbal_motor == &gimbal_control.gimbal_pitch_motor)
		 {
			  gimbal_motor->gimbal_motor_relative_angle_pid.kp = 100.0f;   //15 38
				gimbal_motor->gimbal_motor_relative_angle_pid.ki = 0.1f;    //0
				gimbal_motor->gimbal_motor_relative_angle_pid.kd = 4.0f;    //0
					
	  		gimbal_motor->gimbal_motor_gyro_pid.Kp = 2175.0f; //2900  4500  //3200
				gimbal_motor->gimbal_motor_gyro_pid.Ki = 0.0f;      //60       //5
				gimbal_motor->gimbal_motor_gyro_pid.Kd = 0.0f;       //0     //100						
		 }
    //角度环，速度环串级pid调试
    gimbal_motor->motor_gyro_set = gimbal_PID_calc(&gimbal_motor->gimbal_motor_relative_angle_pid, gimbal_motor->relative_angle, gimbal_motor->relative_angle_set, gimbal_motor->motor_gyro);
	...
	else if(autoaim_mode_flag == 1)   //自瞄的pid
	{
		 	 if(gimbal_motor == &gimbal_control.gimbal_yaw_motor)
	    {
		    gimbal_motor->gimbal_motor_relative_angle_pid.kp = 38.0f;   //15 38          100
				gimbal_motor->gimbal_motor_relative_angle_pid.ki = 0.0f;    //0
				gimbal_motor->gimbal_motor_relative_angle_pid.kd = 0.0f;    //0
				
				gimbal_motor->gimbal_motor_gyro_pid.Kp = 2900.0f; //2900  4500  //3200        5000
				gimbal_motor->gimbal_motor_gyro_pid.Ki = 0.0f;      //60       //5
				gimbal_motor->gimbal_motor_gyro_pid.Kd = 0.0f;       //0     //100			
			}
		 if(gimbal_motor == &gimbal_control.gimbal_pitch_motor)
...
{ 
	if(shoot_control.press_r || (shoot_control.shoot_rc->key.v &KEY_PRESSED_OFFSET_V) )
	{
		autoaim_mode_flag = 1;
		pitch_add_little = 0;
		yaw_add_little = 0;
...
//  if(windmill_key_control->gimbal_rc_ctrl->rc.s[1] == 2 || windmill_key_control->gimbal_rc_ctrl->rc.s[1] == 3)
//	{
//		autoaim_mode_flag = 1;
//	}
