...
//-------------------------------------------------------------------------------------------------------------
//			float walking_diff[4];
//   
//   for (i = 0; i < 4; i++)
//    {
//        walking_diff[i] = (float)(chassis_move_control_loop->motor_speed_pid[1].out - chassis_move_control_loop->motor_chassis[1].chassis_motor_measure->given_current)/(float)chassis_move_control_loop->motor_speed_pid[i].out;
//    }
//  //03???? ???
//    if(walking_diff[0] >0.3 && walking_diff[3] >0.3)
//  {
//   chassis_move_control_loop->motor_speed_pid[1].out+=(chassis_move_control_loop->motor_speed_pid[0].out-chassis_move_control_loop->motor_chassis[0].chassis_motor_measure->given_current);
//   chassis_move_control_loop->motor_speed_pid[2].out+=(chassis_move_control_loop->motor_speed_pid[3].out-chassis_move_control_loop->motor_chassis[3].chassis_motor_measure->given_current);
//  }
//  //12???? ???
//  else if(walking_diff[1] >0.3 && walking_diff[2] >0.3)
//  {
//   chassis_move_control_loop->motor_speed_pid[0].out+=(chassis_move_control_loop->motor_speed_pid[1].out-chassis_move_control_loop->motor_chassis[1].chassis_motor_measure->given_current);
//   chassis_move_control_loop->motor_speed_pid[3].out+=(chassis_move_control_loop->motor_speed_pid[2].out-chassis_move_control_loop->motor_chassis[2].chassis_motor_measure->given_current);
//  }
//------------------------------------------------------------------------------------------------------------
...
static void start_run()
{
	...
	if(flying_flag)
		{
		START_RUN_SEN_X = 0.00003 * 0.5 * 7 * 0.1325;  //shift加到最大速度4.0 
		START_RUN_SEN_Y = 0.000015 * 0.5 * 2.6667;
		}
		if(xtl_flag)
		{
		...
		{
				START_RUN_SEN_X = 0.000015 * 0.5 *(get_robot_level()+ 1.5);  //初始  1级乘1.5   2级乘2   3级乘2.5
				START_RUN_SEN_Y = 0.000015 * 0.5 *(get_robot_level()+ 1.5);
		}
...
//单项赛
	...
	if(flying_flag == 1)
	{
		start_run_speed_x = -START_RUN_SEN_X * (start_time * start_time - 1100 * start_time);
	  start_run_speed_y = -START_RUN_SEN_Y * (start_time * start_time - 1100 * start_time);
	}
	else 
	{
		start_run_speed_x = -START_RUN_SEN_X * (start_time * start_time - 680 * start_time);
	  start_run_speed_y = -START_RUN_SEN_Y * (start_time * start_time - 680 * start_time);
	}
}
...
void chassis_rc_to_control_vector(fp32 *vx_set, fp32 *vy_set, chassis_move_t *chassis_move_rc_to_vector)
{
    ...
    {
			    vx_set_channel = start_run_speed_x*115/100;  //前 115
			    vy_set_channel = 0;
//        vx_set_channel = chassis_move_rc_to_vector->vx_max_speed;
    ...
    {
			    vx_set_channel = -start_run_speed_x*115/100;  //   115
//        vx_set_channel = chassis_move_rc_to_vector->vx_min_speed;
    ...
    {
			    vy_set_channel = start_run_speed_y;  //  115
...
//        vy_set_channel = chassis_move_rc_to_vector->vy_min_speed;
    ...
    {
			    vx_set_channel = start_run_speed_x*80/100;  //
			    vy_set_channel = start_run_speed_y*80/100;  //
//        vx_set_channel = chassis_move_rc_to_vector->vx_max_speed;
    }
...
//				if(fabs(chassis_move_control_loop->motor_chassis[0].speed_set+chassis_move_control_loop->motor_chassis[2].speed_set)<fabs(2*(CHASSIS_WZ_SET_SCALE - 1.0f) * MOTOR_DISTANCE_TO_CENTER * wz_set))
//				{
//					if(chassis_move_control_loop->motor_chassis[0].speed_set>chassis_move_control_loop->motor_chassis[2].speed_set)
//					{
//						chassis_move_control_loop->motor_chassis[1]=chassis_move_control_loop->motor_chassis[1]-fabs(2*(CHASSIS_WZ_SET_SCALE - 1.0f) * MOTOR_DISTANCE_TO_CENTER * chassis_move_control_loop->wz_set)+fabs(chassis_move_control_loop->motor_chassis[0].speed_set-chassis_move_control_loop->motor_chassis[2].speed_set);
//						chassis_move_control_loop->motor_chassis[2]=chassis_move_control_loop->motor_chassis[2]-fabs(2*(CHASSIS_WZ_SET_SCALE - 1.0f) * MOTOR_DISTANCE_TO_CENTER * chassis_move_control_loop->wz_set)+fabs(chassis_move_control_loop->motor_chassis[0].speed_set-chassis_move_control_loop->motor_chassis[2].speed_set);
//					}
//					else
//					{
//						chassis_move_control_loop->motor_chassis[0]=chassis_move_control_loop->motor_chassis[0]-fabs(2*(CHASSIS_WZ_SET_SCALE - 1.0f) * MOTOR_DISTANCE_TO_CENTER * wz_set)+fabs(chassis_move_control_loop->motor_chassis[0].speed_set-chassis_move_control_loop->motor_chassis[2].speed_set);
//						chassis_move_control_loop->motor_chassis[3]=chassis_move_control_loop->motor_chassis[3]-fabs(2*(CHASSIS_WZ_SET_SCALE - 1.0f) * MOTOR_DISTANCE_TO_CENTER * wz_set)+fabs(chassis_move_control_loop->motor_chassis[0].speed_set-chassis_move_control_loop->motor_chassis[2].speed_set);					
//				  }
//				}
    }
	...
	if(gimbal_turn_flag == 1)  //一键掉头
	{
		for (i = 0; i < 4; i++)
		{
			chassis_move.motor_speed_pid[i].Kp = 10000.0f;   //15000
			chassis_move.motor_speed_pid[i].Ki = 10.0f;      //10
			chassis_move.motor_speed_pid[i].Kd = 0.0f;      //0

			chassis_move.chassis_angle_pid.Kp = 65.0f;      //60
			chassis_move.chassis_angle_pid.Ki = 8.0f;            
			chassis_move.chassis_angle_pid.Kd = 2.0f;      
	...
	else if(gimbal_turn_flag == 0)
	{
		for (i = 0; i < 4; i++)
		{
			chassis_move.motor_speed_pid[i].Kp = 15000.0f;   //15000  10000 26000      
			chassis_move.motor_speed_pid[i].Ki = 0.0f;    // 10    10
			chassis_move.motor_speed_pid[i].Kd = 0.2f;     

			chassis_move.chassis_angle_pid.Kp = 15.0f;    //60    25        40
			chassis_move.chassis_angle_pid.Ki = 0.1f;       //6     4       1
			chassis_move.chassis_angle_pid.Kd = 5.0f;     //0      0        3
			PID_calc(&chassis_move_control_loop->motor_speed_pid[i], chassis_move_control_loop->motor_chassis[i].speed, chassis_move_control_loop->motor_chassis[i].speed_set);
		}
	}
